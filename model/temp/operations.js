...
let profile = {
    signin: (obj) => {
        ...
        return Promise.resolve(post(obj.collection)
            ...
            .catch(err => {
                throw {err: err, reason: "failed to connect the collection"}
            })
        )
    },
        ...
        return Promise.resolve(post(obj.collection)
            .then((db) => {
                ...
                profile.save().then(res => {
                    ...
                    throw {err: err, reason: "failed to insert new profile"}
                })
                    .catch(err => {
                        throw {err: err, reason: "failed to insert new profile"}
                    })
        ...
        return Promise.resolve(post(obj.collection)
            .then((db) => {
                db.update({id: obj.id}, {$addToSet: {map: {$each: obj.class}}})
                    ...
                    .catch(err => {
                        throw {err: err, reason: "failed to insert new map"}
                    })
            })
            .catch(err => {
                throw {err: err, reason: "failed to connect the collection"}
